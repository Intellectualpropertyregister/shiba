class RouteRatingService
  ...
  def rate(line)
    ...
    points = refine_points(points, @minimum_grid_size / 2)
    points.map do |point|
      zone_rating = rate_point_by_zone(point)
      if zone_rating then
        unless zone_rating[:daytime_safety_level] and zone_rating[:dark_safety_level] then
          grid_rating = rate_point_by_grid(point)
          zone_rating[:daytime_safety_level] = grid_rating[:daytime_safety_level] unless zone_rating[:daytime_safety_level]
          zone_rating[:dark_safety_level] = grid_rating[:dark_safety_level] unless zone_rating[:dark_safety_level]
        end
        zone_rating
      else
  ...
  def rate_point_by_zone(point)
    ...
    if zone then
      daytime_safety_level = zone.daytime ? SafetyLevelConverter.from_key(zone.zone_type) : nil
      dark_safety_level = zone.dark ? SafetyLevelConverter.from_key(zone.zone_type) : nil

      {
        ...
        col: nil,
        zone_id: zone.id,
        daytime_safety_level: daytime_safety_level,
        dark_safety_level: dark_safety_level
      }
    end
  end
  ...
  def setup_cities
    @cities = City.intersect_with_bounds(@bounding_box[:south_west][:lat], @bounding_box[:south_west][:lng], @bounding_box[:north_east][:lat], @bounding_box[:north_east][:lng]).all
    logger.info "Cover #{@cities.count} cities"
    @cities
  end

  def setup_zones
    @zones = Zone.intersect_with_bounds(@bounding_box[:south_west][:lat], @bounding_box[:south_west][:lng], @bounding_box[:north_east][:lat], @bounding_box[:north_east][:lng]).all
  ...
  def create_grids(city)
    ...
    CrimeGrid.where(city: city).intersect_with_bounds(@bounding_box[:south_west][:lat], @bounding_box[:south_west][:lng], @bounding_box[:north_east][:lat], @bounding_box[:north_east][:lng]).find_each(batch_size: 10000) do |cg|
